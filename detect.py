...
def run(
    ...
    # while True:
    while True:
        res = requests.get("http://192.168.0.116:54664/sample", stream=True)
            ...
            if line:
                ...
                for arr in data["samples"]:
                    ...
                    if len(data_buffer) == 200:
                        ...
                        print(f"{s}{'' if len(det) else '(no detections), '} {confidence} {dt[1].dt * 1E3:.1f}ms")
            #             break
            # if len(data_buffer) == 200:
            #     break
        # break
...
def parse_opt():
    ...
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[96, 640], help='inference size h,w')
